...
// WaveformDisplay
//
// Animated 48-bar waveform. MarketStateData drives:
//   animSpeedScale → controller duration
//   peakScale      → max bar height
...
class _WaveformDisplayState extends State<WaveformDisplay>
  ...
  static const int _kBaseMs = 1400;
  ...
  Duration _duration() {
    return Duration(
      milliseconds: (_kBaseMs / widget.marketState.animSpeedScale).round(),
    );
  }
...
class _WaveformPainter extends CustomPainter {
  ...
  static const List<double> _kRatios = <double>[
  ...
  Color _barColor(double ratio, double opacity) {
    final Color c0 = waveformColors[0];
    final Color c1 = waveformColors[1];
    final Color c2 = waveformColors[2];
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < _kCount; i++) {
      ...
      final double breathe =
          math.sin(animValue * 2.0 * math.pi + i * 0.42) * breatheAmp;
      ...
      if (dist <= highlight + 2 && glowScale > 0.3) {
        final double glowOp = (opacity * 0.18 * glowScale).clamp(0.0, 0.55);
        final Paint glowPaint = Paint()
          ..style = PaintingStyle.fill
          ..maskFilter = MaskFilter.blur(
              BlurStyle.normal, (3.5 * glowScale).clamp(1.0, 8.0))
          ..color = _barColor(ratio, glowOp);
        canvas.drawRRect(
